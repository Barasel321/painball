...
scoreboard players operation @e[type=minecraft:marker,tag=pkd2,limit=1] constant = @s totalkd
...
execute as @s at @s run data modify entity @e[type=minecraft:marker,tag=pkd2,limit=1] CustomName set from block 0 86 -131 Text1

data merge block 0 86 -131 {Text1:'{"selector":"@e[tag=stattest,limit=1]","color":"gold"}'}
execute as @s at @s run data modify entity @e[type=minecraft:marker,tag=pkd2name,limit=1] CustomName set from block 0 86 -131 Text1
